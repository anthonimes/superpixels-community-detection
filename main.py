...
class Segment(object):
    def __init__(self,dirpath,algorithm,dataset,folder,numbers_of_regions,radius,threshold):
        self.dirpath                     = dirpath
        self.algorithm                   = algorithm
        self.numbers_of_regions           = numbers_of_regions
        self.radius                      = radius
    ...
    def __call__(self, filename):
        utils.study(self.algorithm,self.dirpath,filename,self.dataset,numbers_of_regions=self.numbers_of_regions,radius=self.radius,threshold=self.threshold)
            ...
            for dataset,folder in datasets:
                ...
                for radius in radii[algorithm]:
                    for graph_weight_threshold in graph_weight_thresholds[algorithm]:
                        ...
                        segment = Segment(dirpath,algorithm,dataset,folder,numbers_of_regions,radius,graph_weight_threshold)
                        cs=1
                        if len(images) >= jobs:
                            cs=len(images)//jobs
